...
// ─── Launch API (DFY ecommerce) ──────────────────────────────────────────────

export type LaunchAssetKind = "logo" | "store" | "product_page" | "ad" | "ugc" | "email";
export type LaunchAssetStatus = "in_progress" | "in_review" | "ready";
export type LaunchStatus = "active" | "paused" | "completed";
export type LaunchEventKind = "phase_change" | "asset_ready" | "note" | "upcoming";
...
export const launchApi = {
  get: () => apiFetch<LaunchPayload>("/api/launch"),

  create: (businessName: string, niche: string) =>
    apiFetch<{ launch: LaunchSummary; created: boolean }>("/api/launch", {
      method: "POST",
